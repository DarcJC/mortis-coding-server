...
| `mortis-session` | 磁盘 CoW 会话存储：write/delete、`edit_file`（diffy/search-replace）、status、`similar` diff/patch、持久化、TTL 回收。 |
| `mortis-asm` | 汇编查询会话：异步下载二进制（reqwest）、`object` 校验/解析头与符号、`capstone` 反汇编、地址→符号；内存注册表 + 落盘二进制 + TTL 回收。 |
...
max_download_bytes = 268435456 # 单次下载上限（256 MiB）
...
| 会话写文件（整体覆盖） | `PUT /api/v1/sessions/{sid}/file?path=`（请求体=文件字节） |
| 会话 diff/搜索替换写入 | `PATCH /api/v1/sessions/{sid}/file?path=` body `{"diff":"<统一 diff>"}` 或 `{"edits":[{"search","replace","all"}]}`（二者择一） |
...
| 建汇编会话 | `POST /api/v1/asm/sessions` body `{"url":"<http(s)>"}` |
| 列/查/删汇编会话 | `GET /api/v1/asm/sessions` · `GET/DELETE /api/v1/asm/sessions/{aid}`（查=状态/进度/校验结果） |
| 反汇编地址段 | `GET /api/v1/asm/sessions/{aid}/disasm?start=&len=`（`start` 支持 `0x` 十六进制） |
| 地址→函数名 | `GET /api/v1/asm/sessions/{aid}/function?address=` |
| 头/段元数据 | `GET /api/v1/asm/sessions/{aid}/metadata` |
...
# 按 diff 写入（严格 apply；失败返回 409）
curl -X PATCH -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"diff":"--- a/src/lib.rs\n+++ b/src/lib.rs\n@@ -1 +1 @@\n-old\n+new\n"}' \
  "http://127.0.0.1:8080/api/v1/sessions/$SID/file?path=src/lib.rs"

# 按 search/replace 写入（search 须唯一命中，否则 409）
curl -X PATCH -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"edits":[{"search":"fn old()","replace":"fn new()"}]}' \
  "http://127.0.0.1:8080/api/v1/sessions/$SID/file?path=src/lib.rs"

# 汇编会话：建（主机须在 [asm].allowed_hosts 内）-> 轮询状态 -> 反汇编 -> 元数据
AID=$(curl -s -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"url":"https://files.example.com/app"}' http://127.0.0.1:8080/api/v1/asm/sessions \
  | python -c "import sys,json;print(json.load(sys.stdin)['id'])")
curl -H "Authorization: Bearer $TOKEN" "http://127.0.0.1:8080/api/v1/asm/sessions/$AID"          # state: downloading/validating/ready/failed
curl -H "Authorization: Bearer $TOKEN" "http://127.0.0.1:8080/api/v1/asm/sessions/$AID/disasm?start=0x1000&len=64"
curl -H "Authorization: Bearer $TOKEN" "http://127.0.0.1:8080/api/v1/asm/sessions/$AID/function?address=0x1000"
curl -H "Authorization: Bearer $TOKEN" "http://127.0.0.1:8080/api/v1/asm/sessions/$AID/metadata"
```
...
**仓库/搜索/读取**：`list_repos` · `sync_repo` · `search_code` · `read_file` · `blame_file` · `get_history`

**会话（CoW）**：`create_session` · `list_sessions` · `delete_session` · `write_file` · `edit_file`（diff 或 search/replace 写入）· `delete_file` · `session_status` · `session_diff` · `export_patch`

**汇编查询会话**：`create_asm_session` · `list_asm_sessions` · `get_asm_session`（状态/进度）· `delete_asm_session` · `asm_disassemble` · `asm_resolve_function` · `asm_metadata`

每个工具返回 JSON 文本内容（`result.content[0].text` 为序列化后的 JSON 字符串）。`edit_file` 的 `diff`/`edits` 二者择一；`asm_disassemble`/`asm_resolve_function` 的地址参数支持 `0x` 十六进制字符串。
...
- **diff / search-replace 写**（`edit_file`）：在写锁内**原子**地「读当前内容 → 应用 → 写回 upper」（与 `write_file` 共用同一把锁，杜绝竞态读改写）。`diff` 为统一 diff，**严格** apply、上下文不匹配即 `409 Conflict`；`edits` 为字面 search/replace（每块须**唯一**命中，或置 `all=true` 替换全部），缺失/歧义同样报错。仅适用于 UTF-8 文本文件。
...
汇编查询会话用于对**临时二进制**做只读的低层检查：传入 URL → 服务端异步下载 → 校验格式 → 反汇编 / 解析符号与头数据。与 CoW 会话相互独立。

### 生命周期与状态机

`create` 立即返回（状态 `downloading`），下载与校验在后台进行，通过 `get` 轮询：

```
downloading{downloaded,total} ──下载完成──► validating ──解析成功──► ready{info}
        └─（下载失败/超大）─┐                       └─（非法/不支持）─┐
                            └─────────► failed{code,message} ◄───────┘
```

- **downloading**：`downloaded` 实时字节数；`total` 在拿到 `Content-Length` 后给出，否则 `null`。
- **ready**：`info` 为 `BinaryInfo`（`format`/`os`/`arch`/`bits`/`little_endian`/`entry`/`sections`/`segments`/符号与导入导出计数；fat Mach-O 还含 `sub_archs`）。
- **failed**：`code` 为稳定机器码（`download_failed`/`too_large`/`invalid_binary`/…），`message` 为详情。

### 格式识别（`object`）

| 检测到 | `os` | `format` |
|---|---|---|
| PE32/PE32+ | `windows` | `pe` |
| ELF32/ELF64 | `linux`（含 Android） | `elf` |
| Mach-O（thin） | `apple`（macOS/iOS） | `mach_o` |
| Mach-O（fat/universal） | `apple` | `mach_o_fat` |

其它（archive、COFF、WASM 等）一律判为 `invalid_binary`。fat Mach-O 会列出全部 `sub_archs`，并优先选择 arm64、其次 x86_64 的切片用于后续查询。

### 查询（须 `ready`，否则 `409`）

- **反汇编**（`asm_disassemble` / `…/disasm`）：把虚拟地址段映射到文件字节（`object` 的 section/segment 文件范围，`.bss` 等无文件字节的区域报错），交由 `capstone` 反汇编，返回 `{address,bytes(hex),mnemonic,operands}` 列表。支持 x86/x64/ARM/ARM64（小端）。
- **地址→函数名**（`asm_resolve_function` / `…/function`）：在符号表中查找包含该地址的函数符号（精确），否则取最近的前驱符号（`exact=false`）；strip 过的二进制返回 `name=null`。
- **元数据**（`asm_metadata` / `…/metadata`）：直接返回校验阶段缓存的 `BinaryInfo`。

### 安全与回收

- **下载白名单（默认拒绝）**：见[配置](#配置)的 `[asm]`；非白名单主机或空白名单一律 `403`，仅 http/https，超大/超时即 `failed`，**禁用重定向**。
- **所有权**：会话归创建它的 principal；他人访问 `403`。
- **存储与回收**：会话元数据在内存、二进制落盘 `data/asm/<aid>/`；空闲超过 `[asm].session_ttl` 由后台回收器删除。进程重启后汇编会话消失（按需重建）。
...
- **写入内容**：MCP `write_file`/`edit_file` 接受 UTF-8 文本（`edit_file` 仅适用于 UTF-8 文本文件）；二进制整体写入经 REST `PUT`（原始字节）。
- **反汇编范围**：`capstone` 覆盖 x86/x64/ARM/ARM64（小端）；大端 ARM、Thumb 显式模式、调试信息（DWARF/PDB）级别的符号化暂未支持。汇编会话不持久化（重启即失）。
